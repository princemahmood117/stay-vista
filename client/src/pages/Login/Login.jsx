...
const Login = () => {
  const { signIn, signInWithGoogle, setLoading, loading, resetPassword } = useAuth();
  ...
  const handleResetPassword = async () => {
    console.log(email);
    if(!email) {
      return toast.error('please provide an email first')
    }
    try {
      await resetPassword(email)
      toast.success('please check your provided email')
      setLoading(false);
    }
    catch (error){
      console.log(error);
